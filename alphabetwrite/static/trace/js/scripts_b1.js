document.addEventListener("DOMContentLoaded", function() {
    var infoSound =new Audio('/static/trace/info_sound/04_content_infob1.wav');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function uploadImage(blob, imageName, imageNumber) {
        ...
        try {
            const response = await fetch('/trace/lower-b/', {
                method: 'POST',
                body: formData
            });
            if (!response.ok) {
    ...
    function checkAnswers(predicted_label1) {
        ...
        if (isCorrectPredictedLabel1 && !isCorrectStrokeOrder) {
            ...
            document.querySelector('.life4').style.backgroundImage = 'url("/static/trace/img/0ed145.png")';
            localStorage.setItem('life4Color', "/static/trace/img/0ed145.png");
        
            modal.style.display = "block";
                
            // if 정답이면 알파벳 선택하는 menu 페이지로 이동 
            setTimeout(function() {
